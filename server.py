...
class UserAIRequest(BaseModel):
    model: str
    messages: list[CurrentChatContent] # This is kind of a slippery slope, appending all the old messages into the context each time is a little spooky (memory leakscontext accumulation
...
#========================================================
# App Routes
@app.get("/hello", description="Hello world!")
...
def register_zeroconfai(port: int) -> tuple[Zeroconf, ServiceInfo]:
    ...
    service_name = f"Zeroconf.{service_type}"
    ...
    print(f"Please browse for: {service_type}")
