...
def write_port_file(port_file: str, port: int, host: str):
    try:
        with open(port_file, 'w') as f:
            f.write(f"{host}:{port}")
        logger.info(f"Wrote port info to {port_file}")
    except Exception as e:
...
def main():
    ...
    parser.add_argument("--port", type=int, default=9876, help="Port to bind to (auto-detect if not specified)")
    parser.add_argument("--port-file", type=str, default=None, help="File to write port information to")
    args = parser.parse_args()
    
    port = args.port if args.port else find_port_number(args.host)
    
    if args.port_file:
        write_port_file(args.port_file, port, args.host)
